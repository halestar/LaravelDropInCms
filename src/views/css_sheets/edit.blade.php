...
@section('content')
    <div class="container">
        <h1 class="border-bottom d-flex justify-content-between align-items-center">
            {{__('dicms::css_sheets.edit')}}
        </h1>
        <form method="POST" action="{{ \halestar\LaravelDropInCms\DiCMS::dicmsRoute('admin.sites.sheets.update', ['site' => $site->id, 'sheet' => $sheet->id]) }}"  enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
            ...
            <div class="row">
                ...
                <a class="btn btn-secondary col m-2" href="{{ \halestar\LaravelDropInCms\DiCMS::dicmsRoute('admin.sites.sheets.index', ['site' => $site->id]) }}">{{ __('dicms::admin.cancel') }}</a>
            </div>
        </form>
    </div>
@endsection
